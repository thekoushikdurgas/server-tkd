...
router.get("/createtodo", fetchuser, async (req, res) => {
    try {
        const { name, source, description } = req.body;
        const user = await todo.create({ user: req.user.id, name, source, description, completeStatus: 0 });
        var temp1 = {};
        temp1.name = user.name;
        temp1.source = user.source;
        temp1.description = user.description;
        res.json({ success: true, todo: temp1 });
    } catch (error) { console.error(error.message); res.json({ success: false, errors: error.message }); }
    console.log('createtodo');
});
router.get('/todo', fetchuser, async (req, res) => {
    try {
        const temp = await todo.find({ user: req.user.id });
        var temp2 = [];
        for (var i = 0; i < temp.length; i++) {
            var temp1 = {};
            temp1.id = temp[i].id;
        ...
        res.json({ success: true, todo: temp2 });
    }
    catch (error) { console.error(error.message); res.json({ success: false, errors: error.message }); }
    console.log('todo');
});
